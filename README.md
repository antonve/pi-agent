...
- Claimed first-mate sessions keep a dedicated `firstmate-todo` pane on the far right of the claimed tab. A missing pane is created at 25% width; reload, reclaim, and reconciliation preserve subsequent manual resizing. The pane recreates itself if it disappears, restarts its TUI process when needed, and never steals focus during provisioning or recovery.
